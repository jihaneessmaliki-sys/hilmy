import { SOCIAL_CHANNELS, type SocialKey } from '@/lib/social-channels'
import { SocialChannelLink } from './SocialChannelLink'

type ChannelValues = Partial<Record<SocialKey, string | null | undefined>>

interface Props {
  /** Valeurs brutes depuis profiles. Les vides/null sont skippés. */
  values: ChannelValues
  /** "stack" = tous empilés (sidebar contact), "inline" = boutons en ligne (CTA section). */
  variant?: 'stack' | 'inline'
  /** Affiche le label du canal en plus du CTA. */
  showHandle?: boolean
  /** ID du profile ciblé. Si fourni, chaque clic POST /api/track/contact. */
  profileId?: string | null
}

/**
 * Liste de boutons cliquables pour tous les canaux renseignés
 * (whatsapp, phone_public, email, instagram, tiktok, linkedin, facebook,
 * youtube, site_web). Les vides sont automatiquement omis.
 *
 * Si profileId est fourni, chaque clic est tracké via /api/track/contact.
 */
export function SocialChannelsButtons({
  values,
  variant = 'stack',
  showHandle = false,
  profileId = null,
}: Props) {
  const filled = SOCIAL_CHANNELS.map((c) => {
    const raw = values[c.key]
    if (!raw || typeof raw !== 'string' || !raw.trim()) return null
    return { channel: c, value: raw.trim() }
  }).filter((x): x is NonNullable<typeof x> => x !== null)

  if (filled.length === 0) {
    return (
      <p className="text-[13px] italic text-texte-sec">
        Aucun canal renseigné pour l&apos;instant.
      </p>
    )
  }

  const containerClass =
    variant === 'inline'
      ? 'flex flex-wrap gap-2'
      : 'flex flex-col gap-2'

  return (
    <div className={containerClass}>
      {filled.map(({ channel, value }) => {
        const url = channel.toUrl(value)
        const display = channel.displayValue?.(value) ?? value
        const isExternal =
          url.startsWith('http') &&
          !(channel.key === 'email' || channel.key === 'phone_public')
        return (
          <SocialChannelLink
            key={channel.key}
            href={url}
            channelKey={channel.key}
            profileId={profileId}
            isExternal={isExternal}
            className={
              variant === 'inline'
                ? 'group inline-flex items-center gap-2 rounded-full border border-or/30 bg-blanc px-4 py-2 text-[12px] font-medium text-vert transition-colors hover:border-or hover:bg-creme-soft'
                : 'group flex items-center justify-between gap-3 rounded-full border border-or/30 bg-blanc px-4 py-2.5 text-[13px] font-medium text-vert transition-colors hover:border-or hover:bg-creme-soft'
            }
          >
            <SocialIcon channel={channel.key} />
            <span className="flex-1">
              {channel.ctaLabel}
              {showHandle && variant === 'stack' && (
                <span className="ml-2 text-[11px] text-texte-sec">
                  {display}
                </span>
              )}
            </span>
            <span
              className="text-or transition-transform group-hover:translate-x-0.5"
              aria-hidden="true"
            >
              →
            </span>
          </SocialChannelLink>
        )
      })}
    </div>
  )
}

/**
 * Petite icône inline par plateforme. SVG perso (lucide n'a pas tous les
 * brand icons). Hauteur 16px.
 */
function SocialIcon({ channel }: { channel: SocialKey }) {
  const props = {
    width: 16,
    height: 16,
    viewBox: '0 0 24 24',
    fill: 'none',
    stroke: 'currentColor',
    strokeWidth: 1.5,
    strokeLinecap: 'round' as const,
    strokeLinejoin: 'round' as const,
    'aria-hidden': true,
    className: 'text-or shrink-0',
  }
  switch (channel) {
    case 'whatsapp':
      return (
        <svg {...props}>
          <path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" />
        </svg>
      )
    case 'phone_public':
      return (
        <svg {...props}>
          <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
        </svg>
      )
    case 'email':
      return (
        <svg {...props}>
          <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
          <polyline points="22,6 12,13 2,6" />
        </svg>
      )
    case 'instagram':
      return (
        <svg {...props}>
          <rect x="2" y="2" width="20" height="20" rx="5" />
          <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
          <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
        </svg>
      )
    case 'tiktok':
      return (
        <svg {...props}>
          <path d="M9 12a4 4 0 1 0 4 4V4a5 5 0 0 0 5 5" />
        </svg>
      )
    case 'linkedin':
      return (
        <svg {...props}>
          <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-4 0v7h-4v-7a6 6 0 0 1 6-6z" />
          <rect x="2" y="9" width="4" height="12" />
          <circle cx="4" cy="4" r="2" />
        </svg>
      )
    case 'facebook':
      return (
        <svg {...props}>
          <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" />
        </svg>
      )
    case 'youtube':
      return (
        <svg {...props}>
          <path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z" />
          <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02" />
        </svg>
      )
    case 'site_web':
      return (
        <svg {...props}>
          <circle cx="12" cy="12" r="10" />
          <line x1="2" y1="12" x2="22" y2="12" />
          <path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" />
        </svg>
      )
    default:
      return null
  }
}
